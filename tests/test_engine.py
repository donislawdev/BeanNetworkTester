...
def test_the_start_line_about_the_driver_queue_is_filled_in_to_the_last_number():
    ...
    showing, so the line would lose all four numbers and nothing else would notice.
    """
    lines = []
    sh = BeanEngine(log_fn=lines.append)
    sh.start("test", divert=ParamDivert([FakePacket(size=100, port=8402)]))
    sh.stop()

    said = [line for line in lines if "4096" in line and "2000" in line]
